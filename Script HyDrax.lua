...
local Window = WindUI:CreateWindow({
    Title = "<b><font size='26'> Nirv̬an͠a Vo͢id </font></b>", 
    Icon = "rbxassetid://107080351416911", 
    Author = "<font size='17'>       ﷽</font>",
    Folder = "Hx Hub",
    ...
    User = {
        ...
        Callback = function()
            ...
            WindUI:Notify({Title = "HyDrax Owner Script", Duration = 8})
        end,
    },
})
...
ImgBtn.Image = "rbxassetid://107080351416911" 
ImgBtn.ZIndex = 9999
...
Window:Tag({
    Title = "<b><font size='10' color='#000000'>HyDrax</font></b>", 
    Color = Color3.fromHex("#bf0000")
})
        ...
        task.spawn(function()
            while _G.FastPunch do
                ...
                if punch then
                    ...
                    if punch.Parent ~= char and char:FindFirstChild("Humanoid") then char.Humanoid:EquipTool(punch) end
                    punch:Activate()
                end
                if muscleEvent then 
                    muscleEvent:FireServer("punch", "rightHand") 
        ...
        local function stopAnims()
            ...
            if humanoid then
                for _, track in pairs(humanoid:GetPlayingAnimationTracks()) do
                    ...
                    if anim and (blockedAnimations[anim.AnimationId] or name:match("punch") or name:match("attack") or name:match("right")) then track:Stop() end
                end
            end
        end
        _G.AnimMonitorConnection = RunService.Heartbeat:Connect(function()
...
KillSelect:Button({
    ...
    Callback = function()
        ...
        local function overrideToolActivation()
            ...
            if lp.Character then _G.ToolConnections.Character = watchTools(lp.Character) end
        end
        setupAnimationBlocking() 
        overrideToolActivation()
        if not _G.AnimMonitorConnection then
            ...
            task.spawn(function()
                while _G.FastPunch do
                    ...
                    if character then
                        ...
                        if punch then
                            if punch:FindFirstChild("attackTime") then punch.attackTime.Value = 0 end
                            if punch.Parent ~= character and character:FindFirstChild("Humanoid") then character.Humanoid:EquipTool(punch) end
                            punch:Activate()
                        end
                    end
                    task.wait()
    ...
    Callback = function(v)
        ...
        while bringTargetsEnabled do
            ...
            if hand and #killTargets > 0 then
                local t = Players:FindFirstChild(killTargets[1])
                if t and t.Character and t.Character:FindFirstChild("HumanoidRootPart") then t.Character.HumanoidRootPart.CFrame = hand.CFrame end
            end
            task.wait(0.1)
        end
    end
...
local domainWhitelistDropdown = KillSelect:Dropdown({
    ...
    Callback = function(v)
        ...
        for _, opt in ipairs(v) do 
            local n = getName(opt) 
            if n and Players:FindFirstChild(n) then manualWhitelist[Players[n].UserId] = true end 
        end
    end
})
...
local function autoRefreshDropdowns()
    if isRefreshing then return end
    isRefreshing = true
    
    local currentList = getDisplayList()
    whitelistDropdown:Refresh(currentList)
    task.wait(1)
    spyDropdown:Refresh(currentList)
    task.wait(1)
    targetDropdown:Refresh(currentList)
    task.wait(1)
    tpDropdown:Refresh(currentList)
    task.wait(1)
    domainWhitelistDropdown:Refresh(currentList)
...
Players.PlayerAdded:Connect(function()
    task.wait(3)
    task.spawn(autoRefreshDropdowns)
end)
...
function ZorVexStatus.new(mainWindow)
    ...
    self.DropdownNeedsUpdate = false
...
function ZorVexStatus:StartAutoRefresh() 
    ...
    task.spawn(function()
        while true do
            task.wait(3)
            if self.DropdownNeedsUpdate then
                self.DropdownNeedsUpdate = false
                pcall(function() self:UpdateDropdown() end)
            end
        end
    end)
end
